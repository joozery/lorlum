...
import { Plus, X, ChevronDown, FileText, Truck, ArrowLeft } from "lucide-react";
...
const EMPTY_BUYER: BuyerInfo = { name: "", nameEn: "", address: "", taxId: "", phone: "", email: "" };
...
export default function NewPurchasePage() {
  ...
  useEffect(() => {
    fetch("/api/site-settings")
      .then(r => r.json())
      .then(d => { if (d.company) setBuyer({ ...EMPTY_BUYER, ...d.company }); })
      .catch(() => {});
  }, []);

  // Generate the PO number client-side only — Math.random() at render time
  // would mismatch between the server-rendered HTML and the client hydration.
  useEffect(() => {
  ...
  return (
    <div>
      ...
      <main className="p-6 space-y-6">
        ...
        {/* ── Top bar ── */}
        <div className="flex items-center justify-between rounded-xl border border-gray-100 bg-white px-5 py-3.5 shadow-sm">
          <button
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">วันที่: <strong className="text-gray-700">{new Date(issueDate).toLocaleDateString("th-TH")}</strong></p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="rounded-xl border border-gray-100 bg-white">
            {/* Table header */}
            <div className="grid rounded-t-xl bg-gray-900 text-white text-[10px] font-semibold uppercase tracking-wider px-3 py-2.5"
        ...
        <div className="flex items-start justify-between gap-5">
          ...
          {/* Summary box */}
          <div className="w-64 rounded-xl border border-gray-100 overflow-hidden shrink-0 bg-white">
            <div className="divide-y divide-gray-50">
              {[
        ...
        {/* ── Sticky footer ── */}
        <div className="sticky bottom-0 z-10 flex items-center justify-between border-t border-gray-100 bg-white/95 backdrop-blur px-5 py-4 rounded-xl shadow-sm">
