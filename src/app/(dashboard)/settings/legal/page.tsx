...
export default function LegalEditorPage() {
  ...
  const load = useCallback(async (type: DocType) => {
    ...
    const s    = data.sections?.length ? data.sections : DEFAULT_SECTIONS[type];
    setSections(s);
    setLoading(false);
    setDirty(false);
  }, []);
                  ...
                  <div key={i} className="rounded-xl border border-gray-100 bg-white shadow-sm overflow-hidden">
                    ...
                    <div className="flex items-center gap-2 px-4 py-2.5 border-b border-gray-50 bg-gray-50/60">
                      ...
                      <div className="flex gap-1 shrink-0">
                        <button
                          onClick={() => moveSection(i, -1)} disabled={i === 0}
                          className="h-6 w-6 rounded flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 disabled:opacity-20 bg-transparent border-none cursor-pointer text-xs"
                        >↑</button>
                        <button
                          onClick={() => moveSection(i, 1)} disabled={i === sections.length - 1}
                          className="h-6 w-6 rounded flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 disabled:opacity-20 bg-transparent border-none cursor-pointer text-xs"
                        >↓</button>
                        <button
