...
const FIELDS = [
  ...
  { key: "shoulder", label: "Shoulder Width",      placeholder: "e.g. 45" },
  { key: "sleeve",   label: "Sleeve / Body Length", placeholder: "e.g. 63" },
] as const;

type FK = "chest" | "waist" | "shoulder" | "sleeve";
type FS = Record<FK, string> & { note: string };

export default function BespokeShirtPage() {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();

  const [orderNumber, setOrderNumber] = useState("");
  const [hasTrousers, setHasTrousers] = useState(false);
  ...
  const [form, setForm] = useState<FS>({ chest: "", waist: "", shoulder: "", sleeve: "", note: "" });
  ...
  useEffect(() => {
    ...
    fetch(`/api/store/orders/${id}/bespoke`)
      ...
      .then(d => {
        ...
        setHasTrousers(types.includes("trousers"));
        const shirt = (d.bespokeItems ?? []).find((i: { garmentType: string }) => i.garmentType === "shirt");
        if (shirt?.measurements) {
          const m = shirt.measurements;
          setForm({ chest: String(m.chest ?? ""), waist: String(m.waist ?? ""), shoulder: String(m.shoulder ?? ""), sleeve: String(m.sleeve ?? ""), note: String(m.note ?? "") });
        }
      })
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    const { chest, waist, shoulder, sleeve } = form;
    if (!chest || !waist || !shoulder || !sleeve) { setError("Please fill in all measurements."); return; }
    setSaving(true);
    const res = await fetch(`/api/store/orders/${id}/bespoke`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ garmentType: "shirt", chest: Number(chest), waist: Number(waist), shoulder: Number(shoulder), sleeve: Number(sleeve), note: form.note }),
    });
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#F7F2E8", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Jost, sans-serif" }}>
      <p style={{ fontSize: 10, letterSpacing: "0.4em", textTransform: "uppercase", color: "#C9A752", animation: "pulse 2s infinite" }}>Loading…</p>
    </div>
  );
    ...
    <div style={{
      ...
      <nav style={{
        ...
        <ul style={{ display: "flex", gap: 36, listStyle: "none", padding: 0, margin: 0 }} className="bespoke-nav-links">
          ...
          {hasTrousers && <li><Link href={`/account/orders/${id}/bespoke/trousers`} style={{ fontWeight: 300, fontSize: 10.5, letterSpacing: "0.18em", textTransform: "uppercase", color: "#8C7355", textDecoration: "none" }}
            onMouseEnter={e => (e.currentTarget.style.color = "#C9A752")} onMouseLeave={e => (e.currentTarget.style.color = "#8C7355")}>Trousers</Link></li>}
        </ul>
          ...
          {saved ? (
            <div style={{ animation: "rise-in 0.8s cubic-bezier(0.23,1,0.32,1) forwards" }}>
              ...
              <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, fontSize: "clamp(28px,4vw,38px)", color: "#4A3219", lineHeight: 1.15, marginBottom: 20 }}>
                Your Shirt Is Now<br /><em style={{ fontStyle: "italic", color: "#6B4E2A", fontWeight: 400 }}>In the Making</em>
              </h2>
              <p style={{ fontWeight: 300, fontSize: 13, lineHeight: 1.9, color: "#8C7355", maxWidth: 400, margin: "0 auto 32px" }}>
                Your measurements have been sent to our atelier. Our tailoring team will reach out within 3 business days to confirm fit before your shirt goes into production.
              </p>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16, alignItems: "center" }}>
                {hasTrousers && (
                  <Link href={`/account/orders/${id}/bespoke/trousers`} style={{ fontSize: 10, fontWeight: 400, letterSpacing: "0.24em", textTransform: "uppercase", color: "#C9A752", textDecoration: "none", borderBottom: "1px solid #C9A752", paddingBottom: 6 }}
            ...
            <>
              ...
              <p style={{ ...RI(0.4), fontWeight: 300, fontSize: 13.5, lineHeight: 1.95, letterSpacing: "0.02em", color: "#8C7355", maxWidth: 420, margin: "0 auto 34px" }}>
                Thank you for your order. Before our atelier begins cutting your shirt, please provide your measurements below so it can be tailored precisely to you.
              </p>
              ...
              <div style={{ ...RI(0.5), display: "inline-block", fontFamily: "'Cormorant Garamond', serif", fontSize: 13.5, letterSpacing: "0.1em", color: "#4A3219", borderTop: "1px solid rgba(201,167,82,0.3)", borderBottom: "1px solid rgba(201,167,82,0.3)", padding: "10px 24px", marginBottom: 44 }}>
                Order No. <span style={{ color: "#C9A752" }}>{orderNumber}</span> · Bespoke Shirt
              </div>
              ...
              <form style={{ ...RI(0.66), textAlign: "left" }} onSubmit={handleSubmit} noValidate>
                ...
                <div style={{ marginBottom: 34 }}>
                  ...
                  <textarea
                    ...
                    placeholder="e.g. I prefer a slightly looser fit through the shoulders."
