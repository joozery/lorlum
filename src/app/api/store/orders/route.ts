...
export async function POST(req: NextRequest) {
  try {
    ...
    const order = await Order.create({
      ...
      items:         items.map((i: { productId: string; productName: string; imageUrl?: string; color?: string; size?: unknown; price: number; qty: number }) => ({
      ...
      shippingAddress: {
        ...
        city:     shipping?.city     ?? "",
        zip:      shipping?.zip      ?? "",
        country:  shipping?.country  ?? "Thailand",
      },
      note: note ?? "",
    });
