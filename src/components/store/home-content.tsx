      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-espresso">
        ...
        <div className="relative z-10 text-center px-6">
          <span className="block text-[9.5px] font-normal tracking-[0.6em] uppercase text-gold-lt mb-6 anim-rise-1">
            {hero.badge || t.heroSeason}
          </span>
          <h1 className="font-cormorant font-light leading-[0.92] uppercase" style={{ fontSize: "clamp(56px,9vw,140px)" }}>
            <span className="block overflow-hidden">
              <span className="inline-block text-gold-pl anim-rise-2">{hero.headingLine1 || t.heroTitle1}</span>
            </span>
            <span className="block overflow-hidden">
              <span className="inline-block italic text-gold anim-rise-2i">{hero.headingLine2 || t.heroTitle2}</span>
            </span>
          </h1>
          <p className="mx-auto mt-8 max-w-[460px] font-light text-[13.5px] tracking-[0.06em] leading-[1.9] text-gold-pl/70 anim-rise-3">
            {hero.subtext || t.heroDesc}
          </p>
        ...
        <div className="absolute bottom-9 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-2.5 anim-rise-5">
          <span className="text-[8px] tracking-[0.4em] uppercase text-gold-pl/55">{t.heroScroll}</span>
      ...
      <section id="spotlight" className="px-5 md:px-20 py-24 md:py-36 max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 items-center">
        <RevealSection>
          <div className="relative overflow-hidden border border-gold/[0.18]" style={{ aspectRatio: "4/5" }}>
            ...
            <span className="absolute top-5 left-5 z-10 text-[8px] font-normal tracking-[0.22em] uppercase bg-espresso text-gold-lt px-3 py-1.5">{t.spotlightBadge}</span>
          </div>
        </RevealSection>
        <RevealSection>
          <div>
            <span className="block text-[9px] tracking-[0.6em] uppercase text-gold mb-5">{t.spotlightEye}</span>
            {spotlight ? (
              <>
              ...
              <>
                ...
                <p className="font-light text-[13.5px] tracking-[0.03em] leading-[1.95] text-muted max-w-[420px] mb-9">{t.fallbackDesc}</p>
                <div className="flex items-baseline gap-4 mb-10">
                  <span className="font-cormorant font-semibold text-[34px] text-gold">{t.fallbackPrice}</span>
                </div>
      ...
      <section id="atelier" className="bg-cream border-y border-gold/[0.15] px-5 md:px-20 py-24 md:py-36">
        <RevealSection>
          <div className="max-w-[640px] mx-auto text-center mb-20 md:mb-24">
            <span className="block text-[9px] tracking-[0.6em] uppercase text-gold mb-5">{t.atelierEye}</span>
            <h2 className="font-cormorant font-light text-oak-d leading-[1.05]" style={{ fontSize: "clamp(34px,4vw,50px)" }}>
              {t.atelierTitle.split(" to ").length > 1 ? (
                <>{t.atelierTitle.split(" to ")[0]} to <em className="italic text-oak">{t.atelierTitle.split(" to ")[1]}</em></>
              ) : t.atelierTitle}
            </h2>
          </div>
        </RevealSection>
      ...
      <section id="preview" className="px-5 md:px-20 py-24 md:py-36 max-w-[1320px] mx-auto">
        <RevealSection>
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-14 gap-5">
            <div>
              <span className="block text-[9px] tracking-[0.6em] uppercase text-gold mb-4">{t.previewEye}</span>
              <h2 className="font-cormorant font-light text-oak-d leading-none" style={{ fontSize: "clamp(32px,3.6vw,46px)" }}>
                {t.previewTitle1} <em className="italic text-oak">{t.previewTitle2}</em>
              </h2>
            </div>
        ...
        {sorted.length === 0 ? (
          <div className="py-20 text-center">
            <span className="text-[11px] tracking-[0.2em] uppercase text-muted">{t.noProducts}</span>
          </div>
        ) : (
                ...
                <Link href={`/products/${p.id}`} className="no-underline block">
                  <div className="p-card bg-white border border-gold/[0.15] overflow-hidden relative transition-all duration-500 cursor-pointer group">
                    <div className="p-img relative flex items-center justify-center overflow-hidden bg-cream" style={{ aspectRatio: "3/3.5" }}>
                      {i === 0 && <span className="p-badge absolute top-3 left-3 z-[4] text-[8px] tracking-[0.22em] uppercase bg-gold text-espresso px-3 py-1">{t.badgeNew}</span>}
                      {p.featured && i !== 0 && <span className="p-badge absolute top-3 left-3 z-[4] text-[8px] tracking-[0.22em] uppercase bg-espresso text-gold-lt px-3 py-1">{t.badgeFeatured}</span>}
                      {p.imageUrl ? (
      ...
      <section className="bg-espresso px-5 md:px-20 py-20 md:py-32 text-center relative overflow-hidden">
        ...
        <RevealSection>
          <p className="relative z-[1] font-cormorant italic font-light text-gold-pl max-w-[820px] mx-auto mb-7 leading-[1.4]" style={{ fontSize: "clamp(26px,3.2vw,42px)" }}>
            &ldquo;{(() => {
              const parts = t.pressQuote.split(t.pressQuoteHL);
              if (parts.length < 2) return t.pressQuote;
              return <>{parts[0]}<span className="text-gold">{t.pressQuoteHL}</span>{parts[1]}</>;
            })()}&rdquo;
          </p>
